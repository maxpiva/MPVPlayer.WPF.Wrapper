...
namespace HwndExtensions.Adorner
{
    ...
    internal class HwndAdornerGroup : HwndSourceConnector
    {
        ...
        public static Rect GetRectFromRoot(UIElement root)
        {
            Rect rect=new Rect(
            ...
            {
                //remove chrome spacing
                WindowChrome chrome = WindowChrome.GetWindowChrome(win);
                if (chrome != null)
                {
                    rect.X += chrome.GlassFrameThickness.Left + 6;
                    rect.Y += chrome.GlassFrameThickness.Top + 6;
                    rect.Width -= chrome.GlassFrameThickness.Left + 6 + chrome.GlassFrameThickness.Right + 6;
                    rect.Height -= chrome.GlassFrameThickness.Top +6 + chrome.GlassFrameThickness.Bottom + 6;
                }
            }
            return rect;
        }
        ...
        private IntPtr OwnerHook(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            else if (msg == Win32.WM_SIZE && ((int)wParam)==Win32.SIZE_MAXIMIZED)
            {
                SetPosition();
            }
            return IntPtr.Zero;
